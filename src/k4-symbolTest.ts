...
function stressReadStrSweep(n: number) {
  const api = volApi()
  ...
  for (let i = 0; i < n; i++) {
    const v = n > 1 ? i / (n - 1) : 0
    const s = (api.call('str_for_value', fixFloat(v)) as any).toString()
    if (!seen[s]) {
      seen[s] = 1
      distinct++
    }
...
function stressReadStrSame(n: number) {
  const api = volApi()
  const fixed = fixFloat(0.5)
  const t0 = now()
  for (let i = 0; i < n; i++) {
  ...
  p(
    ...
    'identical reads in',
    now() - t0,
    'ms; 1 distinct string (control — should NOT bloat)'
  )
}
...
function stressPathSet(n: number) {
  const api = volApi()
  const t0 = now()
  for (let i = 0; i < n; i++) {
...
function stressObsCreate(n: number) {
  const id = masterVolId()
  const t0 = now()
  for (let i = 0; i < n; i++) {
    const a = new LiveAPI(noFn, '')
    a.id = id
...
function stressObsReuse(n: number) {
  const id = masterVolId()
  const a = new LiveAPI(noFn, '')
  const t0 = now()
  for (let i = 0; i < n; i++) {
...
function anything() {
  ...
  else if (cmd === 'stressPathSet') stressPathSet(parseInt(a[0]) || 10000)
  else if (cmd === 'stressObsCreate') stressObsCreate(parseInt(a[0]) || 1000)
  else if (cmd === 'stressObsReuse') stressObsReuse(parseInt(a[0]) || 1000)
