...
var APPOINTED_DEVICE = 'live_set appointed_device';
...
function isDevice(api) {
    var t = api.type;
    return (0, liveApi_1.apiValid)(api) && t !== 'Song' && t !== 'Track';
}
...
function controlSurfaceConfigured() {
    var s = getScratch();
    s.path = 'live_app';
    var cs = s.get('control_surfaces');
    if (!cs || !cs.length)
        return false;
...
function refreshDeviceSource() {
    var next = controlSurfaceConfigured() && appointedResolves();
    if (next === useAppointed)
        return;
    useAppointed = next;
    log('device source -> ' +
...
var appointedApi = null;
function initAppointedWatcher() {
    appointedApi = new LiveAPI(function (args) {
