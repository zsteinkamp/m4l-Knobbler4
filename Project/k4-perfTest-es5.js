"use strict";
// Performance test: ES5 version for [js] engine (SpiderMonkey)
// Load as [js k4-perfTest-es5] with 1 inlet, 2 outlets
// Send "run" to execute all benchmarks, or individual tests:
...
function benchmark(label, reps, fn) {
    ...
    post_line('  ' + label + ': min=' + result.min + 'ms  median=' + result.median + 'ms  max=' + result.max + 'ms');
    return result;
}
...
function testJsonSerialize(n) {
    ...
    for (var i = 0; i < 100; i++) {
        var params = [];
        for (var j = 0; j < 8; j++) {
            params.push({ idx: j, value: Math.random(), name: 'Param ' + j });
        }
        objects.push({
            id: i,
            name: 'Track ' + i,
...
function testArrayOps(n) {
    ...
    for (var i = 0; i < n; i++) {
        var arr = [];
        for (var j = 0; j < 200; j++) arr.push(j * Math.random());
...
// require() test skipped — dependent modules are now ES2022 and
// cannot be parsed by the [js] SpiderMonkey engine.
...
function runLiveApi() {
    ...
    benchmark('create w/ observer', reps, function () { return testCreateWithObserver(paths, n); });
    benchmark('reuse w/ observer', reps, function () { return testReuseWithObserver(paths, n); });
    post_line('');
}
...
function runRequire() {
    ...
    post_line('(skipped — dependent modules are ES2022, incompatible with [js])');
    post_line('');
}
...
function run() {
    post_line('');
    post_line('########################################');
    post_line('# Performance Test Harness (ES5 / [js])');
    post_line('########################################');
    post_line('');
    runLiveApi();
    runOutlets();
