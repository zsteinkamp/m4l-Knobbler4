...
- 2026-05-24 [v63](https://github.com/zsteinkamp/m4l-Knobbler4/releases/download/v63/Knobbler4-v63.zip) - Arrangement transport control via the new `pos` capability: stream the song-position playhead (throttled ~20Hz) with song length and time signature, and scrub it from the app (`/scrub`). Stream the arrangement loop brace (`loop_start`/`loop_length`) with inbound `/loopStart` + `/loopLength` to drag the loop edges. Add or remove a cue point at the playhead (`/addCuePoint`), rename one (`/renameCuePoint/<idx>`), and jump-and-delete (`/deleteCuePoint`). Reconnecting clients get cue points and position re-pushed.
- 2026-05-22 [v62](https://github.com/zsteinkamp/m4l-Knobbler4/releases/download/v62/Knobbler4-v62.zip) - With app version 2026.5.22 or newer, swap or move a slot's mapping to another slot via the new `swap` capability. XY pairs survive a swap (only a move out of a pair splits it). Fix: detach Bluhand cue-point observers before dropping them.
- 2026-05-21 [v61](https://github.com/zsteinkamp/m4l-Knobbler4/releases/download/v61/Knobbler4-v61.zip) - Massive rearchitecture yielding big leaps in speed and efficiency. Works on Live 12.3 (and earlier) again.
- 2026-05-20 [v60](https://github.com/zsteinkamp/m4l-Knobbler4/releases/download/v60/Knobbler4-v60.zip) - Map any mixer control (volume, pan, sends, mute, crossfader) to a Knobbler slot via the new `/mkMap` OSC command, to support a forthcoming app update. Mute indicators now reflect tracks implicitly muted by another track's solo (`muted_via_solo`). Sidebar mixer observes crossfade assignment changes live. Migrated all JavaScript modules from the legacy `[js]` engine to `[v8]` (V8's JIT and improved garbage collection), along with performance improvements including raw-OSC string output that avoids Max symbol-table bloat.
- 2026-05-09 [v59](https://github.com/zsteinkamp/m4l-Knobbler4/releases/download/v59/Knobbler4-v59.zip) - Support multiple devices on Push 3 Standalone by making the device port a parameter attached to an encoder. Performance improvements by sending single-item batches as raw OSC messages, skipping the `/batch` wrapper overhead.
- 2026-04-28 [v58](https://github.com/zsteinkamp/m4l-Knobbler4/releases/download/v58/Knobbler4-v58.zip) - Honor Live's "Select on Launch" preference when firing clips and scenes from the Knobbler clip view -- previously these always selected the launched clip/scene regardless of the preference. Thanks to u/Jorne555 on Reddit for the report!
